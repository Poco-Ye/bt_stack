...
typedef volatile struct {
	uchar	cccr_sdio_rev;		/* RO, cccr and sdio revision */
	uchar	sd_rev;			/* RO, sd spec revision */
	uchar	io_en;			/* I/O enable */
	uchar	io_rdy;			/* I/O ready reg */
	uchar	intr_ctl;		/* Master and per function interrupt enable control */
	uchar	intr_status;		/* RO, interrupt pending status */
	uchar	io_abort;		/* read/write abort or reset all functions */
	uchar	bus_inter;		/* bus interface control */
	uchar	capability;		/* RO, card capability */

	uchar	cis_base_low;		/* 0x9 RO, common CIS base address, LSB */
	uchar	cis_base_mid;
	uchar	cis_base_high;		/* 0xB RO, common CIS base address, MSB */

	/* suspend/resume registers */
	uchar	bus_suspend;		/* 0xC */
	uchar	func_select;		/* 0xD */
	uchar	exec_flag;		/* 0xE */
	uchar	ready_flag;		/* 0xF */

	uchar	fn0_blk_size[2];	/* 0x10(LSB), 0x11(MSB) */

	uchar	power_control;		/* 0x12 (SDIO version 1.10) */

	uchar	speed_control;		/* 0x13 */
} sdio_regs_t;
...
typedef volatile struct {
	uchar	devctr;			/* device interface, CSA control */
	uchar	ext_dev;		/* extended standard I/O device type code */
	uchar	pwr_sel;		/* power selection support */
	uchar	PAD[6];			/* reserved */

	uchar	cis_low;		/* CIS LSB */
	uchar	cis_mid;
	uchar	cis_high;		/* CIS MSB */
	uchar	csa_low;		/* code storage area, LSB */
	uchar	csa_mid;
	uchar	csa_high;		/* code storage area, MSB */
	uchar	csa_dat_win;		/* data access window to function */

	uchar	fnx_blk_size[2];	/* block size, little endian */
} sdio_fbr_t;
