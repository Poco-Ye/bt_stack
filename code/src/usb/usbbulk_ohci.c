...
/////////////////////////////////
typedef struct{
	int (*s_OhciHostWriteEpx)(int,int,int,uchar *);
	int (*s_OhciHostReadEpx)(int,int,int,uchar *);
}T_OhciHostOps;

extern OhciHostWrite_Proxy(int start_sector,int sector_num,int sector_byte,uchar *WriteBuf);
extern OhciHostRead_Proxy(int start_sector,int sector_num,int sector_byte,uchar *ReadBuf);
int OhciHostWrite_Std(int start_sector,int sector_num,int sector_byte,uchar *WriteBuf);
int OhciHostRead_Std(int start_sector,int sector_num,int sector_byte,uchar *ReadBuf);

T_OhciHostOps OhciHostOps_Proxy = {
	.s_OhciHostWriteEpx = OhciHostWrite_Proxy,
	.s_OhciHostReadEpx = OhciHostRead_Proxy,
};

T_OhciHostOps OhciHostOps_Std = {
	.s_OhciHostWriteEpx = OhciHostWrite_Std,
	.s_OhciHostReadEpx = OhciHostRead_Std,
};

static T_OhciHostOps *pOhciHostOps = NULL;
...
static int usb_bulk_submit(void *cbw,void *csw,u8 *buffer,int len,int direction)
{
	...
	for(i=0;i<5;i++)//5
	{
		err=ohci_host_out_epx(31,(u8*)cbw);
//if(err)x_debug("%d,out:%d\n",i,err);
		...
		{
			rn=len;
			if(direction == BULK_OUT)err=ohci_host_out_epx(len,buffer);
			else err=ohci_host_in_epx(buffer,&rn);
//if(err)x_debug("%d,io:%d,rn:%d\n",i,err,rn);
		...
		rn=13;
		err=ohci_host_in_epx((u8*)csw,&rn);
...
do_error:
		...
		//if(tmpc&0x60)return FS_ERR_DEV_REMOVED;
		if((rHC_RH_PORT_STATUS1&0x103)!=0x103)return FS_ERR_DEV_REMOVED;
...
static int SPC_TestUnit(void)
{
	...
	return usb_bulk_submit(out, out, NULL, 0, BULK_IN);
}
...
///////////////////////////////////////////////////////////////////////////
void usb_deinit_massdev_udiskA(void)			// 
{
	pOhciHostOps = NULL;
}

int usb_enum_massdev_udiskA(void)
{
	int tmpd;
	unsigned int ReservedSectorsNum;

	fat_unmount(FS_UDISK_A);

	memset(&udisk_info, 0, sizeof(udisk_info));

	if(GetHWBranch() == D210HW_V2x )
	{
		pOhciHostOps = &OhciHostOps_Proxy;
		tmpd = OhciHostEnum_Proxy(submit_buf);
		if(tmpd) return tmpd;
	}
	else
	{
		pOhciHostOps = &OhciHostOps_Std;
	...
	
	{
		...
		udisk_info.ops = &udiskA_op;
		tmpd=fat_mount(FS_UDISK_A, &udisk_info);
		if(tmpd<0)return -10;
	}
	return FS_OK;
}
		...
		char *buf, int len)
{
	...
	if(!pOhciHostOps) return FS_ERR_NOINIT;
	if(sector_num > 0xff||
		start_sector<0||
		(start_sector+sector_num>drv->dwSectorTotal))
		return FS_ERR_ARG;
	assert(len >= (sector_num*drv->wSectorSize));
	return pOhciHostOps->s_OhciHostReadEpx(start_sector, sector_num, udisk_info.wSectorSize, buf);
		...
		char *buf, int len)
{
	...
	assert(len >= (sector_num*drv->wSectorSize));
	return pOhciHostOps->s_OhciHostWriteEpx(start_sector, sector_num, udisk_info.wSectorSize, buf);
...
int OhciHostWrite_Std(int start_sector,int sector_num,int sector_byte,uchar *WriteBuf)
{
	return RBC_Write(start_sector, sector_num, WriteBuf);
}

int OhciHostRead_Std(int start_sector,int sector_num,int sector_byte,uchar *ReadBuf)
{
	return RBC_Read(start_sector, sector_num, ReadBuf);
}
