...
typedef enum
{
   ...
   /* this structure specifies the Number of Service Class UUID's that  */
...
#define ISPP_BUFFER_SIZE_CURRENT                                   (SPP_BUFFER_SIZE_CURRENT)
   ...
   /* ISPP_Set_Configuration_Parameters() function to inform the        */
   /* function NOT to change the Default Frame Size.                    */
...
#define ISPP_FRAME_SIZE_CURRENT                                    (SPP_FRAME_SIZE_CURRENT)
   ...
   /* The following defines the Number of Sessions that are supported by*/
   /* the IAP2 Link.                                                    */
   /* * NOTE * This parameter is only used when an IAP2 connection is   */
   /*          established.                                             */
#define ISPP_NUM_SESSIONS_SUPPORTED_DEFAULT                        1

   /* The following structure is used to contain information about the  */
   /* acccessory.  The device information required for IAP and IAP2 are */
   /* different.                                                        */
...
typedef struct _tagISPP_Configuration_Params_t
{
   ...
   unsigned int NumSessionsSupported;
...
typedef struct _tagISPP_Coprocessor_Info_t
{
   ...
   /* The following defines the Bit Mask for Control Messges that are   */
...
BTPSAPI_DECLARATION int BTPSAPI ISPP_Close_Server_Port(unsigned int BluetoothStackID, unsigned int SerialPortID);
   ...
   /* define whether the port will operate in standard SPP mode or WiAP */
   /* mode.  If the following parameters are non-zero/non-NULL, the port*/
   /* will operating in WiAP mode.  Identification Information is a     */
...
BTPSAPI_DECLARATION int BTPSAPI ISPP_Open_Port_Request_Response(unsigned int BluetoothStackID, unsigned int SerialPortID, Boolean_t AcceptConnection, ISPP_Identification_Info_t *IdentificationInformation, unsigned int MaxRxPacketSize);
   ...
   /* The following function is provided to allow a means to add a WiAP */
   ...
   /*          RFCOMM).                                                 */
   /* * NOTE * The UUID that identifies the port as WiAP capable is     */
...
#define ISPP_Un_Register_SDP_Record(__BluetoothStackID, __SerialPortID, __SDPRecordHandle) \
   ...
   /* WiAP more.  If the parameters are non-zero/non-NULL, the port will*/
   /* operating in WiAP mode.  FIDInfo is a pointer to the FID          */
...
BTPSAPI_DECLARATION int BTPSAPI ISPP_Get_Port_Timeout_Value(unsigned int BluetoothStackID, unsigned int SerialPortID, unsigned int *TimeoutValue);
   ...
   /* the the current Timeout Value that is used for outgoing packets.  */
   /* This function accepts as its first parameter, the Bluetooth Stack */
   /* ID of the Bluetooth Stack for which the request is valid for.  The*/
   /* second parameter is the Serial Port ID that identifies the active */
...
BTPSAPI_DECLARATION int BTPSAPI ISPP_Set_Port_Timeout_Value(unsigned int BluetoothStackID, unsigned int SerialPortID, unsigned int TimeoutValue);
   ...
   /* * NOTE * This function is only available when operating in WiAP   */
...
BTPSAPI_DECLARATION int BTPSAPI ISPP_Send_Control_Message(unsigned int BluetoothStackID, unsigned int SerialPortID, Word_t ControlMessageID, Word_t MessageDataLength, const unsigned char  *MessageData);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_ISPP_Send_Control_Message_t)(unsigned int BluetoothStackID, unsigned int SerialPortID, Word_t ControlMessageID, Word_t MessageDataLength, const unsigned char  *MessageData);
#endif

   /* The following function is used to cancel a packet that has been   */
   /* queued for sending, but has yet to be sent.  This function accepts*/
   /* as its first parameter, the Bluetooth Stack ID of the Bluetooth   */
   /* Stack for which the request is valid for.  The second parameter is*/
   /* the Serial Port ID of Port on which the packet is queued.  The    */
   /* third parameter is the Packet ID that was retuned from a call to  */
   /* ISPP_Send_Session_Data() or ISPP_Send_Raw_Data().  If the packet  */
   /* was successfully cancelled, the function return a positive        */
   /* non-Zero PacketID value that is used to uniquely identify the     */
   /* packet.  When the data packet ultimately sent and acknowledged, a */
   /* Send Confirmation event is dispatched with the PacketID and he    */
   /* status of the transaction.                                        */
