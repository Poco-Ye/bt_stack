...
#define BTPS_STRUCTURE_OFFSET(_x, _y)              ((unsigned long)&(((_x *)0)->_y))
...
BTPSAPI_DECLARATION unsigned int BTPSAPI BTPS_StringLength(BTPSCONST char *Source);
   ...
   /* accepts as its parameters a the output string, the format string, */
   /* and variable length arguments.  The function will return the      */
   /* length of the resulting string if successful or a negative value  */
   /* if there is an error.                                             */
BTPSAPI_DECLARATION int BTPSAPI BTPS_SprintF(char *str, BTPSCONST char *format, ...);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_BTPS_SprintF_t)(char *str, BTPSCONST char *format, ...);
#endif
